\subsection{Configure slave GBTx to use master SCA-IC channel}
This setup is required to program a slave via a master SCA-IC channel.
In a typical scenario, the master is connected to a MiniDAQ so that programming
the slave using the MiniDAQ directly is possible.
Follow \autoref{fig:master-sca-ic} to connect a slave GBTx to the SCA-IC channel
of a master GBTx board.
...
\end{tikzpicture}
\captionof{figure}{Schematic for slave to master SCA-IC setup.}
\label{fig:master-sca-ic}
\end{center}
